...
function to_price1(e) {
    if (!e.includes('.') && e !== '')
        return (e + '.00')
    else if (e.includes('.') && e.slice(-2).includes('.'))
        return (e + '0')
    else return e
}
function to_price(e) {
    ...
    if (price.length > 6 && !price.includes(" ")) {
        let start = price.slice(0, price.length - 6);
        let end = price.slice(price.length - 6);
        let tmp = []
        ...
        let beautiful_price = "";
        for (let index = 0, c = tmp.length; index < c; index++) {
...
function Prix(props) {
    ...
    return <input type="text" name="" id={props.id}
        value={props.value}
        onInput={change}
        tabindex={props.tabindex}
        onBlur={() => {
            if (props.id != 'quantite')
                props.setter(to_price(props.value))
        }}
    >
    </input>
}
